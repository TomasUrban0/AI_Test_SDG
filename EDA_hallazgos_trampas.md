# Revisión EDA — Trampas del dataset y análisis pendientes

## PARTE 1 — TRAMPAS DETECTADAS (ordenadas por severidad)

### 🔴 CRÍTICAS — hay que resolverlas antes de modelar

#### Trampa 1. El dataset está ordenado por `Customer_ID` y la tasa de churn cae al final
...
#### Trampa 2. Clase balanceada artificialmente 50/50
...
#### Trampa 3. Valores negativos imposibles
...
#### Trampa 4. Nulos camuflados como `"U"` o `"Z"` en categóricas
...
#### Trampa 5. Multicolinealidad extrema (r > 0.95)
...
#### Trampa 6. Variables dummy inútiles (near-zero variance)
...
#### Trampa 7. Alta cardinalidad en categóricas
...
#### Trampa 9. `income` no es el ingreso real
Es una **variable ordinal 1–9** (decil/bucket), no dólares. Trátala como ordinal o categoría, no como continua.

#### Trampa 10. `dwllsize` y `HHstatin` tienen letras con significados ordinales no documentados
...
#### Trampa 11. `change_mou` / `change_rev` — ¿leakage?
...
#### Trampa 12. `uniqsubs - actvsubs` podría ser leakage parcial
...
#### Trampa 13. Correlaciones individuales muy bajas
...
10. **Reemplazar "U"/"Z" por NaN** y recalcular el % de nulos real (ver Trampa 4).
11. **Tratamiento de outliers**: decidir y documentar (winsorize p99 vs log vs dejar).
12. **Orden del dataset**: verificar si hay patrón temporal (Trampa 1).
...
Las trampas que debes **mencionar explícitamente** (demuestra rigor y sube mucho la nota):
